...
export default function UploadPage({ token }) {
  const [dossierId, setDossierId] = useState(() => `dossier_${Date.now()}`);
  ...
  async function handleAnalyse() {
    ...
    setError("");
    const batch = [...files];
    const currentDossierId = String(dossierId || "").trim() || `dossier_${Date.now()}`;
    setDossierId(currentDossierId);

    for (let i = 0; i < batch.length; i++) {
      setLoadingIndex(i);
      try {
        const uploaded = await uploadDocument(token, batch[i], currentDossierId);
  ...
  return (
    <div className="content-area">
      <div className="panel glass">
        ...
        <div className="input-row two" style={{ marginBottom: 14 }}>
          <input
            value={dossierId}
            onChange={(e) => setDossierId(e.target.value)}
            placeholder="ID dossier (ex: dossier_123)"
          />
          <button className="btn" type="button" onClick={() => setDossierId(`dossier_${Date.now()}`)}>
            Nouveau dossier
          </button>
        </div>
        ...
        {files.length > 0 && loadingIndex === null && (
      ...
      {results.length > 0 && (
        <div className="panel glass">
          <div className="panel-title">
            ...
            <span className="badge badge-neutral" style={{ marginLeft: 10, fontSize: "0.72rem" }}>
              {String(dossierId || "").trim() || "dossier"}
            </span>
          </div>
