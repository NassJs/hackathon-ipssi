  ...
  return (
    <div className="auth-card glass">
      <div className="auth-head">
        ...
        <div>
          <h1>DocuFlow Pro</h1>
          <p>Connecté au backend du groupe</p>
        </div>
      </div>
  ...
  return (
    <div className="stats-grid">
      <div className="stat-card glass">
        ...
        <span className={`badge ${health.ok ? "badge-success" : "badge-error"}`}>{health.mongo}</span>
      </div>

      <div className="stat-card glass">
        <div className="stat-label">Utilisateurs</div>
        <div className="stat-value">{users.length}</div>
        <span className="badge badge-neutral">/users</span>
      </div>

      <div className="stat-card glass">
        <div className="stat-label">Admins</div>
        <div className="stat-value">{admins}</div>
        <span className="badge badge-warning">Rôle élevé</span>
      </div>

      <div className="stat-card glass">
        <div className="stat-label">Base API</div>
    ...
    <div className="layout">
      ...
      <main className="main-content">
        <header className="header glass">
          <div>
            <h2>{welcome}</h2>
            <p>Front connecté à votre backend Node + Mongo</p>
          </div>
                ...
                <div className="panel glass">
                  ...
                  <div className="quick-actions">
                    <button className="btn btn-primary" onClick={() => setAuthMode("register")}>
                      <UserRoundPlus size={14} /> Ajouter un membre
                    </button>
                    <p>
                      Endpoint backend actif : <strong>{API_BASE_URL}</strong>
                    </p>
                    <p>
                      Pour changer l'URL API, ajoute <strong>VITE_API_URL</strong> dans un fichier
                      <strong> .env</strong> du front.
                    </p>
                  </div>
                </div>
              </div>
