...
def _make_facture_data(
    ...
    numero = faker.bothify(text=f"FACT-{today.year}-####")
    emission_dt = faker.date_between(start_date=today - timedelta(days=10), end_date=today)
    echeance_dt = faker.date_between(start_date=emission_dt + timedelta(days=15), end_date=emission_dt + timedelta(days=45))
    date_emission = emission_dt.strftime("%d/%m/%Y")
    date_echeance = echeance_dt.strftime("%d/%m/%Y")
    return FactureData(
        type="facture",
        numero=numero,
...
def _make_devis_data(today: datetime, faker: Faker) -> DevisData:
    numero = faker.bothify(text=f"DEV-{today.year}-####")
    emission_dt = faker.date_between(start_date=today - timedelta(days=10), end_date=today)
    validite_dt = faker.date_between(start_date=emission_dt + timedelta(days=15), end_date=emission_dt + timedelta(days=45))
    date_emission = emission_dt.strftime("%d/%m/%Y")
    date_validite = validite_dt.strftime("%d/%m/%Y")
    montant_ht = float(
        faker.pyfloat(
            left_digits=4,
            right_digits=2,
...
def _make_bon_commande_data(today: datetime, faker: Faker) -> BonCommandeData:
    numero = faker.bothify(text=f"BDC-{today.year}-####")
    signature_dt = faker.date_between(start_date=today + timedelta(days=1), end_date=today + timedelta(days=10))
    date_signature = signature_dt.strftime("%d/%m/%Y")
...
def generate_dossier(
    ...
    devis = _make_devis_data(today, faker)
    # Facture par défaut alignée sur le devis (devis = vérité)
    document = _make_facture_data(today, devis.montant_ht, devis.tva, devis.montant_ttc, faker)
    bon_commande = _make_bon_commande_data(today, faker)
    return DossierData(
...
def main() -> None:
    ...
    parser.add_argument(
        "--ocr-formats",
        default="txt",
        help="Formats OCR à générer (ex: txt,tsv,hocr)",
    )
    parser.add_argument(
        "--no-raw",
        action="store_true",
    ...
    ocr_formats = [f.strip().lower() for f in args.ocr_formats.split(",") if f.strip()]
